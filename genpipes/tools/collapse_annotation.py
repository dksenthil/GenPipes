...
# Modified by GenPipes team: added fallbacks for UCSC GTFs missing gene_type/transcript_type
import numpy as np
import pandas as pd
...
class Annotation:
    def __init__(self, gtfpath):
            ...
            for row in gtf:
                ...
                if annot_type == 'gene':
                    ...
                    if 'transcript_type' not in attributes:
                        attributes['transcript_type'] = attributes.get('gene_type', 'unknown')
        ...
        if any(g.biotype == 'unknown' for g in self.genes):
            print(
                "WARNING: Some genes/transcripts are missing 'gene_type'/'transcript_type' attributes. "
                "This is common with UCSC annotations. For more complete annotation metadata "
                "(gene biotypes, transcript types), consider using GENCODE/Ensembl GTFs. "
                "Proceeding with 'unknown' as the default type."
            )
        if len(self.genes) == 0:
            print(
                "WARNING: No gene-level entries found in GTF. Older UCSC GTFs may only contain "
                "transcript and exon lines without explicit gene records, which will produce "
                "an empty collapsed output. Consider using a GENCODE/Ensembl GTF instead."
            )
...
def add_transcript_attributes(attributes_string):
    ...
    for k in add_list:
        if k not in attr_dict:
            attr_dict[k] = attr_dict.get(k.replace('transcript', 'gene'), '"unknown"')
